/*************************************************************************
	> File Name: pthread_demo.c
	> Author: 
	> Mail: 
	> Created Time: 2015年03月25日 星期三 09时19分35秒
 ************************************************************************/
...
int main(void)
{
    ...
    srand((int)time(0)); //初始化函数发生器

    for (no = 0; no < THREAD_NUM; no++) {
        if (pthread_create(&thread[no], NULL, thrd_func, (void *)no) != 0) {
            //创建THREAD_NUM个线程，传入(void *)no 作为thrd_func的参数
    ...
    for (no = 0; no < THREAD_NUM; no++) {
        ...
        //等thread[no]线程结束，线程函数的返回值放在tret中
