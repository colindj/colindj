/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2015年03月25日 星期三 10时11分31秒
 ************************************************************************/
...
int main(void)
{
    ...
    srand((int)time(0));
    //创建互斥锁（默认），锁上编号返回给mutex
    pthread_mutex_init(&mutex, NULL);

    //创建THREAD_NUM个线程，每个线程号返回给&thread[no]，
    //每个线程的入口函数均为thrd_func,参数为(void *)no
...
void *thrd_func(void *arg) 
{
    int thrd_num = (void *)arg;//传入蚕食，互斥锁的编号
